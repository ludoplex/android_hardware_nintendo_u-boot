...
class TestFsBasic(object):
    def test_fs1(self, u_boot_console, fs_obj_basic):
        ...
        with u_boot_console.log.section('Test Case 1a - ls'):
            ...
            output = u_boot_console.run_command_list(
                [f'host bind 0 {fs_img}', f'{fs_type}ls host 0:0']
            )
            assert re.search(f'2621440000 *{BIG_FILE}', ''.join(output))
            assert re.search(f'1048576 *{SMALL_FILE}', ''.join(output))

        with u_boot_console.log.section('Test Case 1b - ls (invalid dir)'):
            # In addition, test with a nonexistent directory to see if we crash.
            output = u_boot_console.run_command(f'{fs_type}ls host 0:0 invalid_d')
            if fs_type == 'ext4':
    ...
    def test_fs2(self, u_boot_console, fs_obj_basic):
        ...
        with u_boot_console.log.section('Test Case 2a - size (small)'):
            ...
            output = u_boot_console.run_command_list(
                [
                    ...
                    f'{fs_type}size host 0:0 /{SMALL_FILE}',
                    'printenv filesize',
                    'setenv filesize',
                ]
            )
        ...
        with u_boot_console.log.section('Test Case 2b - size (/../<file>)'):
            ...
            output = u_boot_console.run_command_list(
                [
                    f'{fs_type}size host 0:0 /SUBDIR/../{SMALL_FILE}',
                    'printenv filesize',
                    'setenv filesize',
                ]
            )
    ...
    def test_fs3(self, u_boot_console, fs_obj_basic):
        ...
        with u_boot_console.log.section('Test Case 3 - size (large)'):
            ...
            output = u_boot_console.run_command_list(
                [
                    f'host bind 0 {fs_img}',
                    f'{fs_type}size host 0:0 /{BIG_FILE}',
                    'printenv filesize',
                    'setenv filesize',
                ]
            )
