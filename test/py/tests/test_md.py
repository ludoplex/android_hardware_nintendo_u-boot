...
def test_md(u_boot_console):
    ...
    addr = '%08x' % ram_base
    val = 'a5f09876'
    expected_response = f'{addr}: {val}'
    u_boot_console.run_command(f'mw {addr} 0 10')
    response = u_boot_console.run_command(f'md {addr} 10')
    assert expected_response not in response
    u_boot_console.run_command(f'mw {addr} {val}')
    response = u_boot_console.run_command(f'md {addr} 10')
...
def test_md_repeat(u_boot_console):
    ...
    expected_response = f'{addr_repeat}: '
    assert(expected_response in response)
